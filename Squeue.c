...
int squeue_driver_release(struct inode *inode, struct file *file)
{
	...
	kfree(squeue_devp->message_queue[i]);
	}*/
	
	/* Re-Initializing to original value */
	squeue_devp->head = 0;
...
{	
	...
	squeue_devp->message_queue[squeue_devp->tail] = kmalloc(sizeof(struct message), GFP_KERNEL);
	if (!squeue_devp->message_queue[squeue_devp->tail]) {
		printk("Bad Kmalloc\n"); return -ENOMEM;
	}
	else{
		...
		/*UPDATE TAIL AND HEAD FOR NEXT DATA*/
		squeue_devp->tail++;
		squeue_devp->size++;
	}

	/*	UPDATE QUEUE LENGTH	 TO AVOID OVERFLOW OF ARRAY	*/
           ...
           size_t count, loff_t *ppos)
{
	...
	//kfree(squeue_devp->message_queue[squeue_devp->head]);

	/*	UPDATE QUEUE LENGTH	 TO AVOID UNDERFLOW OF ARRAY	*/
